...
**启动前必须确认**：请在你所使用的 LLM 服务后台查询该模型的实际 RPM（每分钟请求数）和 TPM（每分钟 Token 数）限制，并在 Web 界面的侧边栏「LLM 配置」中正确填写。

**不设置或设置不当的后果：**

| 问题 | 原因 | 解决方法 |
|------|------|----------|
| 请求大量返回 429 / 限速错误 | RPM/TPM 设置值高于服务商实际限额 | 降低到服务商官方限制值 |
| 调用速度极慢或卡住 | RPM/TPM 设置值远低于实际限额，系统过度限速 | 提高到与实际限额接近的值 |
| 频繁超时 | 并发数过高或模型响应慢 | 降低 RPM 或调大并发超时时间 |

**常见服务默认限额参考：**

| 服务商 | 模型 | RPM | TPM |
|--------|------|-----|-----|
| SiliconFlow | Qwen/Qwen2.5-7B-Instruct | 3000 | 500000 |
| SiliconFlow | Qwen/Qwen2.5-72B-Instruct | 100 | 50000 |
| SiliconFlow | gpt-3.5-turbo（代理） | 3000 | 1500000 |
| 其他服务 | 请在服务后台查询 | - | - |

> **提示**：不同模型、不同服务商的限额差异极大，请勿直接套用默认值。
DB_TYPE=sqlite
...
| RPM | 每分钟最大请求数（**必须与实际模型限额一致**） | 3000 |
| TPM | 每分钟最大 Token 数（**必须与实际模型限额一致**） | 500000 |
| Temperature | 生成随机性（0~2） | 0.7 |

> **RPM / TPM 注意事项**
> - 不同模型、不同服务商的限额差异极大（例如 7B 模型通常 RPM=3000，72B 模型可能只有 RPM=100）
> - 请在你所使用的 LLM 服务后台查询实际限额，并在侧边栏「LLM 配置」中填写
> - **设置过高**：大量请求返回 429 限速错误
> - **设置过低**：请求排队过慢，模拟耗时极长
> - 系统会根据 RPM/TPM 自动计算最佳并发数，限速错误直接返回不重试
